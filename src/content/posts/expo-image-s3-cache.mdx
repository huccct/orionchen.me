...
summary: 从列表进入详情时，同一张图片为什么还会重新加载？用一次真实优化讲清客户端内存与磁盘缓存、HTTP Cache-Control、不可变 URL 和 S3 SigV4 如何接成完整链路。
...
为了让这个请求头成为不可省略、不可随意修改的上传契约，实现同时修改了三个地方：
...
不过，这些测试没有证明真实设备快了多少，也没有覆盖 CDN 和对象存储最终返回的响应头。现有实现没有基准数据，所以不能严谨地写成“加载速度提升 80%”。要验证线上收益，还应做四件事：
