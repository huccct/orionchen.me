...
@theme {
  ...
  --color-accent: oklch(0.55 0.1 50); /* sienna #A0522D */
...
export function EarlyContentBanner() {
  return (
    <div className="my-6 border-l-2 border-[var(--color-accent)] py-2 pl-4 font-mono text-sm text-[var(--color-muted)]">
      // early content — written years ago, kept for archive completeness; tone & opinions may be
      dated.
    </div>
  )
}
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html
      lang="zh-CN"
      className={`${geistSans.variable} ${geistMono.variable} ${newsreader.variable} ${notoSerifSC.variable}`}
  ...
  return (
    <header className="sticky top-0 z-10 border-b border-[var(--color-border)] bg-[var(--color-bg)]/80 backdrop-blur">
      <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-4 md:px-8">
        <Link href="/" className="font-mono text-sm hover:text-[var(--color-accent)]">
          orionchen.me
        </Link>
...
export function SiteFooter() {
  return (
    <footer className="mt-16 border-t border-[var(--color-border)]">
      <div className="mx-auto flex max-w-5xl flex-col justify-between px-4 py-8 font-mono text-xs text-[var(--color-muted)] md:flex-row md:px-8">
        <span>
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-4">
        ...
        <time className="font-mono text-xs whitespace-nowrap text-[var(--color-muted)]">
          {post.date}
        </time>
      </div>
      {post.summary && (
    ...
    <div className="space-y-16">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {featured.map((w) => (
            <WorkCard key={w.slug} work={w} hasBody={!!w.body} />
          ))}
        </div>
      </section>
...
export function WorksFilter({ works }: { works: (Work & { hasBody: boolean })[] }) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {filtered.map((w) => (
          <WorkCard key={w.slug} work={w} hasBody={w.hasBody} />
        ))}
      </div>
    </>
  )
...
export const mdxComponents: MDXComponents = {
  img: (props: any) => <Image {...props} width={1200} height={675} className="my-6 rounded-md" />,
  h2: (p: any) => <h2 className="mt-8 mb-4 font-serif text-2xl" {...p} />,
  h3: (p: any) => <h3 className="mt-6 mb-3 font-serif text-xl" {...p} />,
  a: (p: any) => (
    <a className="text-[var(--color-accent)] underline-offset-4 hover:underline" {...p} />
  ),
  pre: (p: any) => (
    <pre
...
export function generateStaticParams() {
  return allWorks
    .filter((w) => w.body && String(w.body).trim().length > 0)
    .map((w) => ({ slug: w.slug }))
}
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const staticRoutes = ['', '/works', '/blog', '/tags', '/guestbook', '/about'].map((p) => ({
    url: `${u}${p}`,
    changeFrequency: 'monthly' as const,
  }))
  const posts = allPosts
    .filter((p) => !p.draft)
    .map((p) => ({ url: `${u}/blog/${p.slug}`, lastModified: p.date }))
